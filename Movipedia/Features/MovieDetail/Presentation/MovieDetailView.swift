...
struct MovieDetailView: View {
    ...
    init(movieId: Int) {
        _viewModel = StateObject(wrappedValue: MovieDetailInjection.provideMovieDetailViewModel(movieId: movieId))
    }

    var body: some View {
        Group {
            if viewModel.isLoading || (viewModel.movieDetail == nil && viewModel.errorMessage == nil) {
                LoadingView("Loading movie details...")
            } else if let error = viewModel.errorMessage, viewModel.movieDetail == nil {
                ErrorView(message: error) { viewModel.loadMovieDetail() }
        ...
        .sheet(isPresented: $showTrailerPlayer) {
            if let key = viewModel.officialTrailer?.key {
        ...
        ZStack(alignment: .bottomLeading) {
            KFImage(movie.backdropURL)
                ...
                .overlay(
                    ...
                    .blendMode(.multiply) // optional, gives stronger darkening
                )
    ...
    private var trailerSection: some View {
        Group {
            if viewModel.isLoadingVideos {
                ...
                .padding(.horizontal, 16)
            } else if let trailer = viewModel.officialTrailer {
                VStack(alignment: .leading, spacing: 12) {
                    sectionHeader("Trailers")
    ...
    private var reviewsSection: some View {
            ...
            if viewModel.isLoadingReviews {
                ...
                LazyVStack(spacing: 16) {
                    ForEach(viewModel.reviews) { review in
                        reviewCard(review)
                            .onAppear {
                                viewModel.loadNextReviewsIfNeeded(currentItem: review)
                            }
                    }

                    if viewModel.isLoadingNextReviews {
        ...
        .padding(.horizontal, 16)
        .onAppear { viewModel.loadReviewsIfNeeded() }
    }
